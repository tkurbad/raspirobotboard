    ...
    def test(self, motors = True, leds = True, rangers = True,
        ...
        if matrix:
            ...
            self.ledMatrix.display_string_scrolling(
                u'0123456789abcdefghijklmnopqrstuvwxyzäöüß*@\\:,=!#>([<-%.|+?)];/ ~_;):(:|:/:)',
...
class TorBotThreadController(concurrent.futures.ThreadPoolExecutor):
    ...
    def _get_ranger(self, ranger, iterations = 3, pause = 0.05):
    ...
    def test(self):
        ...
        concurrent.futures.wait(self.rangingThreads, timeout = 1.0)
